...
- A hosted pilot smoke workflow in [`.github/workflows/pilot-smoke.yml`](.github/workflows/pilot-smoke.yml) that reuses the base proxy and header overlay smoke targets plus the report-path site-rejection variants on manual dispatch and a weekly schedule
...
- The validation and deployment docs now distinguish between hosted base plus report-path site-rejection automation and the broader manual overlay smoke matrix
