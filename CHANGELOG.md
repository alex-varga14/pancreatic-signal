...
- HL7 ORU imports now decode base64 `ED` report text, normalize repeated `OBX-5` values, and respect custom `MSH-2` component and repetition separators before triage
...
- API validation reported `114 passed`
...
- `apps/api/.venv/bin/python -m pytest apps/api/tests/test_imports.py apps/api/tests/test_import_runs.py -q` passed on 2026-03-24 with `37 passed`, including inline `Reference.identifier` FHIR coverage plus custom `MSH-2` HL7 delimiter and audit-visibility coverage
