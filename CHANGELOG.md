...
- A bundled hosted smoke evidence builder in [`scripts/build_pilot_smoke_evidence.py`](scripts/build_pilot_smoke_evidence.py) plus `make pilot-smoke-evidence` so downloaded hosted summary artifacts can be consolidated into release-ready JSON and Markdown evidence
...
- The repo now includes a checked-in deidentified retrospective-style external benchmark sample with reproducible JSON, Markdown, and submission artifacts, plus optional `report_excerpt` support so external casebooks can show deidentified report text context
- The public `/proof` surface now renders that checked-in retrospective-style sample alongside the demo comparison, so outside collaborators can inspect both the synthetic casebook and the less-synthetic external sample in one reproducible web view
...
- `apps/api/.venv/bin/python -m pytest apps/api/tests/test_external_evaluation.py apps/api/tests/test_benchmark_submission.py -q` passed on 2026-03-25 with `9 passed`, covering the retrospective-style sample pack, optional report excerpts, and the richer external benchmark template plus casebook-shaped bundle outputs
- `make benchmark-external-sample` and `make refresh-external-sample-proof` both wrote reproducible retrospective-style benchmark artifacts on 2026-03-25
