...
- The public GitHub Actions API now reports three historical `Pilot Smoke` workflow runs from 2026-03-25, all failed push-triggered executions with zero jobs and zero artifacts, so hosted attachment-backed FHIR confirmation is still pending and there is still no usable hosted summary evidence yet
...
- What is still missing is a recorded successful GitHub-hosted execution of that FHIR-only hosted slice so the repo has both local and hosted evidence for the new fixture; the current public baseline is three failed historical runs without jobs or artifacts, not a usable hosted confirmation.
...
- uses the bundled `pilot-smoke-evidence.json` and `pilot-smoke-evidence.md` outputs from `make pilot-smoke-evidence` to turn downloaded artifact summaries into a copyable FHIR confirmation record plus HL7 hosting decision text
