...
This repository is no longer in early MVP scaffolding. The core research prototype is implemented and validated, and the best next work is now Phase 6 hardening around structured adapter failed-run shared-visibility parity on top of the now-comprehensive import smoke coverage.
...
- The latest strict validation pass was rerun after the failed-run shared-visibility change set and remains green
...
- `make validate-strict` passed after adding live site-scope rejection smoke coverage, shared helper assertions, and new overlay targets
...
Proceed with structured adapter failed-run shared-visibility smoke coverage for non-site failures.

### Why this is next

- The generic report path now covers both successful shared visibility and failed shared visibility for a persisted `validation_error` run.
- The remaining access-matrix gap is on the structured adapters. FHIR `unsupported_payload` and HL7 `parse_error` runs prove owner access today, but they still do not prove whether a second correctly scoped operator can inspect those run-scope-only failures.
- Closing that gap would finish the allow/allow side of failed-run visibility across the current import surfaces, not just the generic report endpoint.

### Target outcome

Add a live smoke flow that:
- creates at least one persisted structured-adapter failed import run whose visibility should be granted by actor site scope rather than imported sites
- confirms the creating actor can read the failed run detail
- confirms a second scoped actor with appropriate site visibility can read the same failed run through both audit endpoints
- keeps the existing generic failed-run shared-visibility, successful shared-visibility, and denial-path smoke targets working

### Suggested implementation shape

1. Extend the existing smoke helper with a narrowly scoped structured failed-run shared-visibility mode rather than introducing a new harness.

2. Reuse the current adapter failure paths and audit routes:
   - `/api/v1/imports/fhir/diagnostic-reports`
   - `/api/v1/imports/hl7/oru`
   - `GET /api/v1/imports/runs`
   - `GET /api/v1/imports/runs/{run_id}`

3. Prefer failure buckets with empty `imported_sites` but a populated actor scope, such as:
   - FHIR `unsupported_payload`
   - HL7 `parse_error`

4. Keep the smoke assertions operational:
   - the primary actor still resolves correctly
   - the failed response returns a persisted `run_id`
   - the owning actor can inspect the run
   - the alternate actor sees the same failed run only when site visibility should allow it

5. Prefer additive make targets and docs updates over any new runtime abstractions.

### Acceptance criteria

- At least one live pilot smoke path verifies structured failed-run audit visibility rules across two actors for a non-site failure bucket.
- The smoke output confirms the run ID and the expected allow/allow behavior.
- Deployment docs explain how to run the new structured failed-run shared-visibility smoke.
- `make validate-strict` passes.
...
This is a clean checkpoint. The repo is runnable, validated, and already beyond MVP scaffolding. Import metadata preservation, import-run audit trails, config overrides, the web import workspace, concrete proxy plus header-auth pilot packaging, and live generic successful shared-visibility, generic failed shared-visibility, FHIR/HL7 success, site-scope rejection, report-path parse/validation, adapter-specific malformed-import, and audit-visibility denial smoke coverage in both pilot auth modes are complete, so the next agent should focus on structured adapter failed-run shared-visibility parity rather than missing product fundamentals.
