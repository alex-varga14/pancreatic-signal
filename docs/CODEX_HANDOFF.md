...
- Published external proof packs are now registry-driven through `docs/examples/published-external-benchmarks.json`, so future checked-in collaborator or deidentified benchmark stories can be added to the web proof surface without more one-off page wiring.
...
- The highest-value remaining work is no longer bootstrapping or late-Phase-6 smoke capture. The repo-side Phase 6D release polish is already in place, the first benchmark-realism proof expansion is complete, the external starter pack matches that reviewer-facing proof shape, and the repo now includes a checked-in retrospective-style multi-cohort sample, so the next work should move toward broader collaborator-supplied or deidentified benchmark inputs and reviewer-evaluation depth instead of reopening parser or smoke-baseline work.
- The next benchmark-proof slice should focus on adding another real or collaborator-supplied external pack through the new registry flow rather than hardcoding another single-sample proof panel.
...
- `apps/api/.venv/bin/python -m pytest apps/api/tests/test_external_evaluation.py apps/api/tests/test_benchmark_submission.py -q` passed with `9 passed`, covering the richer external benchmark template, optional report excerpts plus cohorts, and the retrospective-style multi-cohort sample pack plus casebook-shaped external bundle outputs
...
- The repo now also includes a 12-case deidentified retrospective-style multi-cohort sample pack with report excerpts, reviewer cues, cohort coverage, one intentional follow-up miss, and one visible pancreatitis-confounder overcall, and checked-in JSON plus Markdown outputs that demonstrate the same casebook structure on a less-synthetic external dataset; `/proof` now renders that sample alongside the demo comparison instead of leaving it docs-only
