...
Proceed with the next recommended slice from the handoff: HL7 escape-sequence normalization using the message escape character.
...
Target outcome:
- Add at least one meaningful escaped-content HL7 ORU coverage path driven by the message escape character.
- Preserve import metadata, audit visibility, and site-scope semantics across that escape-normalized path.
...
- Prefer a small escape-normalization helper over a parser rewrite, and keep the finding versus impression section assembly behavior understandable.
